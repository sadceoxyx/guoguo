...
/**
 *
 * Create custom admin bar
 * 
 * @global object $wp_admin_bar
 * @return void 
 */
function deals_admin_bar() {
    global $wp_admin_bar;
    ...
    $wp_admin_bar->add_menu(array(
       'parent' => 'wpdeals',
        'id' => 'wpdeals_settings',
        'title' => 'Settings',
        'href' => admin_url('/admin.php?page=wpdeals')
    ));
    ...
    $wp_admin_bar->add_menu(array(
        ...
        'id' => 'wpdeals_codex',
        'title' => 'Codex',
        'href' => 'http://wpdeals.me/wordpress-deals-codex/'
    ));
}
add_action('admin_bar_menu','deals_admin_bar');
...
function deals_image_create_barcode($barcode_data, $barcode_name) {
    ...
    $img_barcode_path = WP_CONTENT_DIR.'/wpdeals/invoices/';
...
function deals_is_empty_stock($post_id=null) {
    global $post;
...
function deals_featured($limit = 1){
    $sticky = get_option( 'sticky_deals', array() );
...
/**
 * Check the featured deals, return true if not empty.
 *
 * @return boolean 
 */
function deals_check_featured(){
    $sticky = get_option( 'sticky_deals', array() );
    if(empty($sticky))
        return false;
...
function deals_count_download($post_id) {
    ...
    if($post_type == 'daily-deals') {
     
        $download_count_before = get_post_meta($post_id,'_download_count',true);
    
        update_post_meta($post_id,'_download_count',$download_count_before+1);
        add_post_meta($post_id,'_download_date',date('c')); //not unique
