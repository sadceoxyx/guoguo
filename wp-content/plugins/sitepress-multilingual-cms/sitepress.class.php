...
class SitePress{
    ...
    function __construct(){
        ...
        if($this->settings['existing_content_language_verified']){
            ...
            if($pagenow == 'edit.php'){
                add_action('admin_enqueue_scripts', array($this, 'edit_page_script'));
            }
    ...
    function edit_page_script(){
        wp_enqueue_script('add_post_params', ICL_PLUGIN_URL . '/res/js/add_post_params.js');
    }
        ...
        if('post-new.php' == $pagenow){
            if(isset($_GET['trid'])){
                ...
                if(isset($_GET['trid']) && ($this->settings['sync_ping_status'] || $this->settings['sync_comment_status'])){
                    $orginal_content = $wpdb->get_row("SELECT post_title, post_content FROM {$wpdb->prefix}posts WHERE ID = {$_GET['post']}");
                    if($orginal_content){ ?>
                        <script type="text/javascript">addLoadEvent(function(){
                            jQuery('#title').val('<?php echo $orginal_content->post_title?>');
                            jQuery('#content').val('<?php echo $orginal_content->post_content?>');
                    });</script>
                   <?php } ?>
                    <?php
