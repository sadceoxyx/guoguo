 ...
 */
function padd_theme_widget_facebook_likebox($w=250,$h=250,$conn=10,$stream=0,$header=0) {
			...
			?>
				<li>
					<a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>">
						<?php 
							$padd_image_def = catch_that_image();
                                                        $exclude[] = get_the_ID();
							if (has_post_thumbnail()) {
								the_post_thumbnail(PADD_THEME_SLUG . '-related-posts', array('title' => get_the_excerpt()));
							} else if ($padd_image_def){
								echo '<img class="thumbnail" title="' . strip_tags(get_the_title()) . '" src="' . $padd_image_def . '" />';
							}else{
                                                            $padd_image_def = get_template_directory_uri() . '/images/thumbnail.png';
                                                            echo '<img class="thumbnail" title="' . strip_tags(get_the_title()) . '" src="' . $padd_image_def . '" />';
                                                        }
...
function catch_that_image() {
  ...
  if(empty($first_img)){ //Defines a default image
    $first_img = get_template_directory_uri() . '/images/thumbnail.png';
  }
  return $first_img;
}
			...
			<?php 
				...
				if (has_post_thumbnail()) {
					...
					//echo '<img class="thumbnail" src="' . $padd_image_def . '" />';
                                        echo '<img class="thumbnail" src="' . catch_that_image() . '" />';
				}
			?>
			</a>
		</li>
